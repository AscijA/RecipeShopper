...
begin
  ...
  values (extensions.digest(code, 'sha256'), p_snapshot::jsonb)
...
as $$
  ...
  where code_hash = extensions.digest(upper(regexp_replace(p_share_code, '[^A-Z0-9]', '', 'g')), 'sha256');
$$;
...
as $$
  ...
  where id = p_workspace_id
    ...
    and code_hash = extensions.digest(upper(regexp_replace(p_share_code, '[^A-Z0-9]', '', 'g')), 'sha256');
$$;
